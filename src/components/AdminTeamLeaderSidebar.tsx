                ...
                <div className="relative flex flex-col h-full overflow-hidden" style={{ width: sidebarWidth }}>
                    ...
                    <div className="p-6">
                        <div className="flex items-center gap-3 px-2 py-1">
                            <div className="relative group shadow-2xl shadow-emerald-500/20">
                                <div className="absolute -inset-1.5 bg-gradient-to-tr from-emerald-600 to-teal-400 rounded-xl blur opacity-25 group-hover:opacity-40 transition duration-500"></div>
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-[0.15em] mt-1.5 opacity-80">
                                    {t('administration_label')}
                                </span>
                            </div>
                        </div>
                    </div>
                                ...
                                <div key={index}>
                                    {isDropdown ? (
                                        <div className="space-y-0.5">
                                            ...
                                            >
                                                <Icon className={`w-[22px] h-[22px] flex-shrink-0 ${isDropdownOpen ? 'text-emerald-600' : 'text-slate-500'}`} strokeWidth={1.5} />
                                                        ...
                                                        <div className="mt-1 space-y-0.5 pl-4">
                                                            {item.subItems?.map((subItem: any, subIndex: number) => {
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        <SubIcon size={18} strokeWidth={1.5} className={isSubActive ? 'text-emerald-600' : 'text-slate-400'} />
                                        ...
                                        >
                                            <Icon className={`w-[22px] h-[22px] flex-shrink-0 ${isActive ? 'text-emerald-600' : 'text-slate-500'}`} strokeWidth={1.5} />
                                            <span className="text-[15px] whitespace-nowrap flex-1">
                                                {item.label}
                                            </span>
                                            {item.badge > 0 && (
                                                <span className="flex items-center justify-center min-w-[20px] h-5 px-1.5 bg-emerald-600 text-white text-[10px] font-black rounded-full shadow-lg shadow-emerald-500/20">
                                                    {item.badge}
                                                </span>
                                            )}
                                        </Link>
