...
interface KeeperModel22SigningModalProps {
    record: RequestRecord;
    onClose: () => void;
    onFinalized: () => void;
}

export default function KeeperModel22SigningModal({ record, onClose, onFinalized }: KeeperModel22SigningModalProps) {
        ...
        const fetchName = async () => {
            try {
                ...
                if (userDoc.exists()) {
                    ...
                    setKeeperFullName(d.fullName || d.displayName || d.username || '');
                }
            } catch { /* ignore */ }
        };
        fetchName();
    ...
    const handleFinalizeSign = async () => {
        if (!signatureData) {
            alert('እባክዎ ከማረጋገጥዎ በፊት ይፈርሙ። (Please sign before finalizing)');
            return;
        }
        if (!db) return;
        ...
        try {
            ...
            // 2. Update Send_to_Users with keeper signature and completed status
            const sendToUserRef = doc(db!, 'Send_to_Users', record.id);
            batch.update(sendToUserRef, {
                status: 'handout_completed',
                keeperSignature: signatureData,
                keeperName: keeperFullName
            });

            // 3. Update Request_materials
            ...
            alert('Failed to finalize handout.');
        } finally {
            ...
            >
                ...
                <div className="flex justify-between gap-8 mb-6">
                    ...
                    <div className="flex-1 text-[11px] space-y-3 font-semibold w-[400px]">
                        {[
                            { am: '1. ገንዘብ ወጪ መዝገብ የተመዘገበት ተራ ቁጥር', en: 'Item No. in Expenditure Registry' },
                            { am: '2. ዕቃ ገቢ መዝገብ የገባበት ገጽ', en: 'No. of entry in register of incoming goods' },
                            { am: '3. ዕቃው የተሰጠው መደብ', en: 'Classification of Stock' },
                            { am: '4. ዕቃው የተቀመጠበት መጋዘን ቁጥር', en: 'Store No.' },
                            { am: '5. የመደርደሪያው ቁጥር', en: 'Shelf No.' },
                            { am: '6. ዕቃው የወጣበት የወጪ መዝገብ ተራ ቁጥር', en: 'No. of entry in register of outgoing goods' }
                        ].map((item, i) => (
                            <div key={i}>
                                <div className="flex items-end">
                                    <span className="shrink-0 whitespace-nowrap">{item.am}</span>
                                    <div className="flex-1 border-b-[0.5px] border-black ml-3 h-[18px]"></div>
                                </div>
                ...
                <div className="text-[13px] leading-[26px] mb-8 font-semibold w-full pr-4 text-justify">
                    ...
                    <div className="flex items-baseline mt-2">
                        ...
                        <span className="flex-1 border-b border-black text-[#0033aa] font-[Kalam] text-[16px] text-center mx-2">{department}</span>
                ...
                <table className="w-full border-collapse border-[1.5px] border-black mb-10">
                    ...
                    <tbody>
                        {displayRows.map((row, idx) => {
                            if (!row.materialName) {
                                return (<tr key={idx}><td className={tdClasses}>{idx + 1}</td><td className={`${tdClasses} text-left px-2`}></td>{Array(10).fill(0).map((_, ci) => <td key={ci} className={tdClasses}></td>)}</tr>);
                            }
                            const mat: any = materialDetails[row.materialCode || ''] || materialDetails[row.materialName?.trim().toLowerCase() || ''] || {};
                            const qty = Number(row.quantity) || 0;
                            const rawUP = Number(mat.unitPrice) || 0;
                            const upBirr = rawUP > 0 ? Math.floor(rawUP).toString() : '';
                            const upCents = rawUP > 0 ? Math.round((rawUP % 1) * 100).toString().padStart(2, '0') : '';
                            const tot = rawUP * qty;
                            const tBirr = tot > 0 ? Math.floor(tot).toString() : '';
                            const tCents = tot > 0 ? Math.round((tot - Math.floor(tot)) * 100).toString().padStart(2, '0') : '';
                            return (
                                <tr key={idx}>
                                    ...
                                    <td className={tdClasses}></td><td className={tdClasses}></td>
                                    <td className={tdClasses}><span className="font-[Kalam] text-[17px] font-bold">{row.quantity}</span></td>
                                    <td className={tdClasses}><span className="font-[Kalam] text-[#0033aa] text-[15px]">{upBirr}</span></td>
                                    <td className={tdClasses}><span className="font-[Kalam] text-[#0033aa] text-[12px]">{upCents !== '00' ? upCents : ''}</span></td>
                                    <td className={tdClasses}><span className="font-[Kalam] text-[#e11d48] text-[15px] font-bold">{tBirr}</span></td>
                                    <td className={tdClasses}><span className="font-[Kalam] text-[#e11d48] text-[12px] font-bold">{tCents !== '00' ? tCents : ''}</span></td>
                ...
                <div className="flex justify-between mt-8 px-10">
                    ...
                    {/* Recipient's Signature — shows employee's saved signature */}
                    <div className="flex flex-col items-center">
                        <div style={{ width: 220, marginBottom: 4 }}>
                            <span style={{ fontWeight: 700, fontSize: 12 }}>{employeeName}</span>
                        </div>
                        <div style={{ width: 220, height: 70, borderBottom: '2px solid #000', position: 'relative' }}>
                            {record.recipientSignature ? (
                                // eslint-disable-next-line @next/next/no-img-element
                                <img src={record.recipientSignature} alt="Recipient Signature" style={{ maxHeight: '100%', maxWidth: '100%', display: 'block', margin: '0 auto' }} />
                            ) : (
                                <span style={{ display: 'block', textAlign: 'center', color: '#ccc', paddingTop: 20 }}>No Signature</span>
                            )}
                        </div>
                ...
                <div className="mt-10 flex justify-center print-hide">
                    <button
                        ...
                        style={{
                            ...
                            background: isSubmitting || !signatureData ? '#86efac' : '#16a34a',
                    ...
                    >
                        {isSubmitting ? (
                            ...
                            <><FiCheck size={18} /> Process &amp; Finalize</>
                        )}
                    </button>
                </div>
